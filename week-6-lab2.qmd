...
1.  What do you think each row represents?
    -   Each row represents dempraphics and water safety in a given district in a given fiscal year
...
4.  Create a scatter plot of violations against race (percent Black population), ethnicity (percent Hispanic population), and SES (median household income). What visualization issues do we get with a scatter plot? How could you address that?\
    -   The points are overplotted i.e., too many are overlapping to see the trend. We can summarize within bins to view the patterns
...
drinking_water_binned <- drinking_water |>
  ...
  group_by(pcthisp) |>
  summarise(violation = mean(violation))
...
1.  Plot the residuals. What pattern do you notice?
    -   These residuals are not normally distributed
...
drinking_water_complete |>
  ...
  ggplot(aes(x = pcthisp, y = residual)) +
  geom_point()
...
```{r}
# 10 to 20 %
inv_logit(beta0_hat + beta1_hat*20) - inv_logit(beta0_hat + beta1_hat*10)

# 45 to 55 %
inv_logit(beta0_hat + beta1_hat*55) - inv_logit(beta0_hat + beta1_hat*45)

# 80 to 90 %
inv_logit(beta0_hat + beta1_hat*90) - inv_logit(beta0_hat + beta1_hat*80)

# doing it all at once
(inv_logit(beta0_hat + beta1_hat*c(20, 55, 90))) - (inv_logit(beta0_hat + beta1_hat*c(10,45,80)))
```
...
**our model is really sensitive to the intercept (beta0) we choose. our uncertainty for beta 0 is really low! with beta 1, big changes lead to really smaller changes in likelihood, leaving our confidence interval high.**
