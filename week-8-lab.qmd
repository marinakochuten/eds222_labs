...
# State hypotheses (done)

# Calculate point estimate

no_outlier_lm <- lm(feather_ug_g_pbwt ~ blood_ug_dl_pbwt,
                        no_outlier)
summary(no_outlier_lm)

# Quantify the uncertainty (estimate + SE)

beta1_est_no_outlier <- summary(no_outlier_lm)$coefficients[2, 1]
beta1_se_no_outlier <- summary(no_outlier_lm)$coefficients[2, 2]

tibble(beta1 = seq(-(beta1_est_no_outlier + beta1_se_no_outlier),
                   (beta1_est_no_outlier + beta1_se_no_outlier),
                   length.out = 200),
       density = dnorm(beta1, mean = 0, sd = beta1_se_no_outlier)) |>
  ggplot(aes(beta1, density)) +
  geom_line(color = "cornflowerblue") +
  geom_vline(xintercept = beta1_est_no_outlier, color = "firebrick")

# Calculate p value

pval_no_outlier <- 2 * pnorm(-abs(beta1_est_no_outlier), mean = 0, sd = beta1_se_no_outlier)
pval_no_outlier

# Interpret
# We can reject the null hypothesis because 0.0033 < 0.05. 
```

**Question:** Of the two analyses (with and without the outlier), which had a lower p-value? Does that make it a better analysis?

**with the outlier had a lower p value. no, this does not make it a better analysis.**
...
# Extract our estimates for beta0, beta1, and sigma (SD of predictions)

beta0_estimate <- summary(no_outlier_lm)$coefficients[1, 1]
beta1_estimate <- summary(no_outlier_lm)$coefficients[2, 1]
sigma_estimate <- summary(no_outlier_lm)$sigma
...
# Verify coefficients match inputs
c(beta0_estimate, beta1_estimate)
summary(lm(feather_ug_g_pbwt ~ blood_ug_dl_pbwt, data = blood_feather_pop))
```
...
# Calculate point estimate and standard error FROM THE SAMPLE
