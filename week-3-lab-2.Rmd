...
# The water temperature regression model fits the data better, which is what I expected, because there is less variance in the water temp scatter plot
```
...
```{r}
# sse is the sum of square of the errors - resid will give all errors for a model
crab_size_errors <- resid(lm(size ~ latitude, pie_crab))
sse <- sum(crab_size_errors^2)
...
summary(lm(hwy ~ year_fct, mpg_fct))

# The intercept (value at the reference level) tells you that the average mpg in 1999 is about 23.43
# The coefficient estimate (the difference relative to the reference level) on the 2008 vintage indicator tells you that 2008 is on average 0.0256 mpg higher than 1999
```

4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

**Answer:** The model suggests that fuel efficiency has not evolved much over time.
...
2.  Second, add two regression lines to the plot, one that shows the predicted relationship between miles per gallon and engine size for the 1999 vintage, and a second that shows the same predicted relationship but for the 2008 vintage.[^week-3-lab-2-1]

[^week-3-lab-2-1]: Hint: The variable `.fitted` gives you the predicted (or "fit") values for all observations in the data.
